...
def get_cwe(description):
    cwe_dict = {'CWE-79': ['cross site scripting', 'xss', 'cross-site scripting'],
...
for json_file_name in json_file_list:
    ...
    print(len(parsed_json['CVE_Items']))
    count = 0
    for cve_items in parsed_json['CVE_Items']:
        cve_entry = cve_items['cve']
        cve_nodes = cve_items['configurations']['nodes']
        count += 1
        if(count % 1000 == 0):
            print(count)
        ...
        get_cwe_ret = get_cwe(cve.description)
        ...
        for node in cve_nodes:
            ...
            if(multi_node_flag == 1):
                for node in nodes:
                    vendor_names = [cpe_match['cpe23Uri'].split(':')[3] for cpe_match in node['cpe_match'] ]
                    # remove the '
                    ...
                    for vendor_name, product_name in zip(vendor_names, products):
                        ...
                        for version_value in product_versions:
